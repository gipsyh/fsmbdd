        ...
        + BitXor<&'b BM::Bdd, Output = BM::Bdd>,
{
    fn build_schedule(trans: &[BM::Bdd], vars: Vec<usize>) -> Vec<Vec<usize>> {
        let mut res = Vec::new();
        let mut vars: HashSet<usize> = HashSet::from_iter(vars.into_iter());
    ...
    fn compute_affinity(a: &BM::Bdd, b: &BM::Bdd) -> f64 {
        let a: HashSet<usize> = HashSet::from_iter(a.support_index().into_iter());
        let b = HashSet::from_iter(b.support_index().into_iter());
